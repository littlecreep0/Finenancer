...
object StubData {
    ...
    var portfolioCashList: List<Cash> = listOf(
        Cash(1, "USD", 100.0)
    )

    var CashDetailsList: List<CashDetails> = listOf(
        CashDetails(1, "USD", 100.0, 100.0, 1.0, "USD", Date())
    )

    var portfolioStockList: List<Stock> = listOf(
        Stock(1, "GazProm", 23_673_512_900.0, 20_000.0)
    )

    var StockDetailsList: List<StockDetails> = listOf(
